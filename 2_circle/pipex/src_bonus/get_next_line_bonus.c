...
#ifndef BUFFER_SIZE
# define GNLB_BUFFER_SIZE 1024
#else
# define GNLB_BUFFER_SIZE BUFFER_SIZE
#endif
...
static char	*ft_handle_read(int fd, char *buf)
{
	...
	str = (char *)malloc(GNLB_BUFFER_SIZE + 1);
	if (str == 0 && buf != 0)
		return (ft_free(&buf));
	if (str == 0)
		return (0);
	...
	{
		rd_bytes = read(fd, str, GNLB_BUFFER_SIZE);
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || GNLB_BUFFER_SIZE <= 0 || fd > GNLB_MAX_FD)
		exit(EXIT_FAILURE);
	*status = GNL_NORMAL;
