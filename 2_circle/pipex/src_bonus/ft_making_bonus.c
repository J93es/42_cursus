...
void	ft_making(int argc, char **argv, char **envp, t_tools *tools)
{
	...
	if (tools->flag_hd == FTTRUE && read(0, temp, 0) < 0)
		ft_reading_empty(tools);
	else if (tools->flag_hd == FTTRUE)
		ft_reading_hd(tools);
	else if (tools->flag_hd == FTFALSE && tools->input_file == -1)
		ft_reading_empty(tools);
	else if (tools->flag_hd == FTFALSE)
		dup2(tools->input_file, STDIN_FILENO);
	while (tools->cmd_start_index < argc - 2)
		ft_cmding(argv[(tools->cmd_start_index)++], envp, tools);
	tools->flag_check_file = FTTRUE;
	dup2(tools->output_file, STDOUT_FILENO);
	ft_output(argv[argc - 2], envp, tools);
}
